...
# UI.js
Designer to create a simple settings menu
...
- When id is transmitted, an empty line will be generated by an auto-id.
- In the UI object, setters / getters on id widgets are automatically created.
- Getter added for Select`<name>Text`reading
- For Color added setter and getter`<name>Int`number

You can refer to the widget as`ui.widget(id)`and access:
